...
class data_collection(Resource):
    ...
    @api.response(200,'200 Data already exist.')
    ...
    def post(self):
        ...
        if len(temp_id) == 0:
            result_dict['location'] = "/yyz/1"
            result_dict['collection_id'] = 1
        else:
    ...
    @api.response(404,'No data exist.')
...
class collection_delete(Resource):
    ...
    @api.response(404,'Incorrect collection_id.')
    @api.doc(description='The collection id must be a number.')
    def get(self,collection_id):
        ...
        if not collection_id.isdigit() or int(collection_id) not in result_id:
            return {"message": "Collection {} is not in the databaset or the input is not a number.".format(collection_id)}, 404
        ...
        for result in c:
            ...
            show_list['indicator_value'] = result[3]
...
class retrieve_top_bottom(Resource):
    ...
    @api.param('q','The q is an optional parameter which can be topN or bottomN and  N can be an integer value between 1 and 100.')
