...
class data_collection(Resource):
    ...
    def post(self):
        ...
        if 'message' in original_data_dict[0]:
            if original_data_dict[0]['messgae'][0]['key'] == 'Invalid value':
                return '{} does not exist.'.format(indicator_id),404

        result_dict = {}
        result_dict['location'] = "/yyz/"+str(indicator_id)
        result_dict['collection_id'] = indicator_id
        result_dict['indicator'] = indicator_id
        ...
        sqlstr = "insert into ASS2 (COLLECTION_ID,LOCATION,INDICATOR,INDICATOR_VALUE,CREATION_TIME,ENTRIES) " \
                 "VALUES('%s','%s','%s','%s','%s','%s'); " % (indicator_id,result_dict['location'], indicator_id,result_dict['indicator_value'],result_dict['creation_time'], entries)
        c.execute(sqlstr)
        conn.commit()
        conn.close()
        show = {}
    ...
    def get(self):
        ...
        for m in temp_item:
            ...
            for item in c:
                temp['location'] = item[2]
                temp['collection_id'] = item[1]
                temp['indicator'] = item[3]
                temp['creation_time'] = item[5]
            result.append(temp)
...
class collection_delete(Resource):
    ...
    def get(self,collection_id):
        ...
        if collection_id not in result_id:
        ...
        show_list = {}
        for result in c:
            show_list['location'] = result[2]
            show_list['collection_id'] = result[3]
            show_list['creation_time'] = result[5]
            show_list['indicator'] = result[3]
            show_list['entries'] = json.loads(result[6])
...
class retrieve_indicator(Resource):
    ...
    def get(self,collection_id,year,country):
        ...
        if collection_id not in result_id:
            return {"message": "Collection {} is not in the databaset/ invalid input".format(collection_id)}, 404
        c.execute("select ENTRIES from ASS2 where COLLECTION_ID ='%s'" % (collection_id))
        result = {}
        result["collection_id"] = collection_id
        result["indicator"] = collection_id
        for i in c:
            temp = str(json.loads(i[0])).split("[")[1].split("]")[0].split('},')
            for item in temp:
                if item[-1] != '}':
                    item = eval(item + '}')
                else:
...
class retrieve_top_bottom(Resource):
    ...
    def get(self,collection_id,year):
        ...
        if collection_id not in result_id:
            return {"message": "Collection {} is not in the databaset/ invalid input".format(collection_id)}, 404
        c.execute("select INDICATOR_VALUE,ENTRIES from ASS2 where COLLECTION_ID ='%s'" % (collection_id))
        result = {}
        temp_list = []
        result['collection_id'] = collection_id
        for i in c:
            result['indicator_value'] = i[0]
